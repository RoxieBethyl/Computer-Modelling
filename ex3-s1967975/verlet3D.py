...
UUN number: s1967975
 ...
 CompMod Ex3: velocity Verlet time integration of a particle moving with the 
 morse potential.
...
def force_dw(D, r_e, alpha, r_12):
    ...
    r_12hat = r_12/r_12n
    return (2 * alpha * D * (1-np.exp((-1)*alpha*(r_12n-r_e))) * np.exp((-1)*alpha*(r_12n-r_e))) * r_12hat
...
def main():
    """
    A simulation is conducted by using the Verlet velecity algorithm which 
    reads input from a given file and outputs the data into another file.
    This simulation updates the using the 2nd order taylor expansion of the 
    position and it's corresponding 1st order expansion for the velocity.
    ...
    time = 0.0        # Set up simulation parameter for time
    ...
    i_energy = p3d.sys_kinetic(particle_list) + pot_energy_dw(D, r_e, alpha, r_12)
                                            # Get initial energy
    force = force_dw(D, r_e, alpha, r_12)   # Get initial force on particle 1
    ...
    for i in range(numstep):
        ...
        r_12 = np.subtract(p2.update_pos_2nd(dt, -force), p1.update_pos_2nd(dt, force))
            
        # Update force
        force_new = force_dw(D, r_e, alpha, r_12)
        ...
        outfile.write("{0:f} {1:f} {2:12.8f}\n".format(time, np.linalg.norm(r_12), energy))
        ...
        #Max and Min energy is found and stored
        if energy > i_energy:
            E_max = float(energy)
    ...
    print(i_energy)
    print(E_max/i_energy)
    print(E_min/i_energy)
    # Post-simulation:
    ...
    pyplot.title('Velocity Verlet: Difference vs Time')
    pyplot.xlabel('Time')
    pyplot.ylabel('Particle postion differnce')
    ...
    print("Frequency: ", f)
    print("Wave number: ", (f/(c*(100)))) # Diving f by c(in meters)
