...
def set_initial_positions(rho, particles):
    '''
    Modifies the positions of all of those particles, and also returns the box
    size to use.
    
    Parameters:
        rho(flaot): number density in particles / o^3
        particles(array): list of particles

    Returns:
        box size(numpy array): Size of the box.
        full_lattice(boolean): list of all modified particles
    '''
...
def set_initial_velocities(mytemp, particles, seed=None):
    '''
    Modifies the velocity of all of those particles randomly according to the
    temperature.

    Parameters:
        mytemp(): Temperature in units of episilion.
        particles(array): list of particles
    '''
